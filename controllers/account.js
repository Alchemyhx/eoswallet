...
module.exports = {
    ...
    accountCreate: async (ctx) => {
        console.log(JSON.stringify(ctx.request.body))
        let {creator, account, activepubkey, ownerpubkey, accountram, accountcpu, accountnet, wallet, password } = ctx.request.body
        ...
        let data = await eos.transaction(tr => {
            ...
            tr.buyrambytes({
                payer: creator,
                receiver: account,
                bytes: parseInt(accountram)
            })
    
            tr.delegatebw({
                from: creator,
                receiver: account,
                stake_net_quantity: accountnet+' EOS',
                stake_cpu_quantity: accountcpu+' EOS',
                transfer: 0
            })
        })
